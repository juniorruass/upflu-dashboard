  ...
  return (
    <>
      ...
      <style>{`
        ...
        .step-card { background: var(--up-bg); border: 1px solid var(--up-border); border-radius: 8px; padding: 14px 16px; margin-bottom: 8px; display: flex; gap: 12px; align-items: flex-start; }
      ...
      <div className="seq-wrap">
        <div style={{ marginBottom: "28px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
          <div>
            ...
            <h2 style={{ fontSize: "24px", fontWeight: "700", color: "var(--up-text)", margin: 0 }}>Sequências</h2>
            <p style={{ fontSize: "13px", color: "var(--up-text-dim)", margin: "6px 0 0" }}>Configure múltiplos follow-ups em dias diferentes</p>
          </div>
        ...
        {/* Form */}
        {showForm && (
          <div style={{ background: "var(--up-bg)", border: `1px solid ${ACCENT}`, borderRadius: "10px", padding: "18px", marginBottom: "20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
              <span style={{ fontSize: "13px", fontWeight: "600", color: "var(--up-text)" }}>Novo passo</span>
              <button onClick={() => setShowForm(false)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--up-text-dim)" }}><X size={15} /></button>
            </div>
            <form onSubmit={addStep} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <div>
                  <label style={{ fontSize: "11px", color: "var(--up-text-dim)", display: "block", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Sequência</label>
                  <input value={form.sequence_name} onChange={(e) => setForm((f) => ({ ...f, sequence_name: e.target.value }))} placeholder="Padrão" style={inputStyle} />
                </div>
                <div>
                  <label style={{ fontSize: "11px", color: "var(--up-text-dim)", display: "block", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Enviar no dia</label>
              ...
              <div>
                <label style={{ fontSize: "11px", color: "var(--up-text-dim)", display: "block", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Mensagem</label>
        ...
        {loading ? (
          ...
          <div key={seq} style={{ marginBottom: "28px" }}>
            <p style={{ fontSize: "11px", color: "var(--up-text-dim)", textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 10px", paddingBottom: "8px", borderBottom: `1px solid var(--up-border)` }}>
              Sequência: {seq}
            </p>
            {steps.filter((s) => s.sequence_name === seq).map((step, i) => (
              <div key={step.id} className="step-card">
                <div style={{ minWidth: "40px", textAlign: "center", paddingTop: "2px" }}>
                  ...
                  <div style={{ fontSize: "10px", color: "var(--up-text-dim)", marginTop: "4px" }}>Dia {step.day_offset}</div>
                </div>
