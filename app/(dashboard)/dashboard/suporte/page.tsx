...
const STATUS = {
  ...
  closed:      { label: "Fechado",      color: "var(--up-text-dim)"    },
};

const PRIORITY = {
  low:    { label: "Baixa",   color: "var(--up-text-dim)"    },
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .ticket-card { background: var(--up-bg); border: 1px solid var(--up-border); border-radius: 8px; padding: 14px 16px; margin-bottom: 8px; cursor: pointer; transition: border-color 0.15s; }
        .ticket-card:hover, .ticket-card.active { border-color: rgba(255,255,255,0.18); }
        .ticket-card.active { border-color: ${ACCENT}; }
        .detail { width: 360px; flex-shrink: 0; background: var(--up-bg); border: 1px solid var(--up-border); border-radius: 10px; padding: 20px; overflow-y: auto; height: fit-content; max-height: calc(100vh - 120px); }
      ...
      <div style={{ padding: "24px 32px 0", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <h2 style={{ fontSize: "24px", fontWeight: "700", color: "var(--up-text)", margin: 0 }}>Suporte</h2>
        </div>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          {(["all", ...Object.keys(STATUS)]).map((s) => (
            <button key={s} onClick={() => setFilterStatus(s)} style={{
              background: filterStatus === s ? "rgba(0,207,255,0.08)" : "transparent",
              border: `1px solid ${filterStatus === s ? ACCENT : "var(--up-border)"}`,
          ...
          <button onClick={fetch_} style={{ background: "transparent", border: `1px solid var(--up-border)`, borderRadius: "6px", padding: "6px 10px", cursor: "pointer", color: "var(--up-text-dim)", display: "flex", alignItems: "center" }}>
            <RefreshCw size={13} />
          </button>
        </div>
      </div>
              ...
              <div key={t.id} className={`ticket-card${selected?.id === t.id ? " active" : ""}`} onClick={() => { setSelected(t); setAdminNotes(t.admin_notes ?? ""); }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: "10px" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: "14px", fontWeight: "600", color: "var(--up-text)", marginBottom: "4px" }}>{t.title}</div>
                    {t.clients && <div style={{ fontSize: "11px", color: "var(--up-text-dim)", marginBottom: "6px" }}>{t.clients.name}</div>}
                    {t.description && <div style={{ fontSize: "12px", color: "var(--up-text-dim)", lineHeight: 1.4 }}>{t.description.slice(0, 100)}{t.description.length > 100 ? "..." : ""}</div>}
                  </div>
        ...
        {selected && (
          <div className="detail">
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
              <h3 style={{ fontSize: "15px", fontWeight: "600", color: "var(--up-text)", margin: 0 }}>{selected.title}</h3>
              <button onClick={() => setSelected(null)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--up-text-dim)" }}><X size={16} /></button>
            </div>

            {selected.clients && <div style={{ fontSize: "12px", color: "var(--up-text-dim)", marginBottom: "12px" }}>Cliente: {selected.clients.name}</div>}
            {selected.description && <p style={{ fontSize: "13px", color: "var(--up-text-muted)", marginBottom: "16px", lineHeight: 1.5 }}>{selected.description}</p>}

            <div style={{ marginBottom: "12px" }}>
              <label style={{ fontSize: "11px", color: "var(--up-text-dim)", textTransform: "uppercase", letterSpacing: "0.1em", display: "block", marginBottom: "5px" }}>Status</label>
              <select value={selected.status} onChange={(e) => updateTicket(selected.id, { status: e.target.value })}
                style={{ width: "100%", background: "var(--up-card)", border: `1px solid var(--up-border)`, borderRadius: "6px", padding: "8px 10px", fontSize: "13px", color: "var(--up-text)" }}>
                {Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
            </div>

            <div style={{ marginBottom: "14px" }}>
              <label style={{ fontSize: "11px", color: "var(--up-text-dim)", textTransform: "uppercase", letterSpacing: "0.1em", display: "block", marginBottom: "5px" }}>Prioridade</label>
            ...
            <div style={{ marginBottom: "12px" }}>
              <label style={{ fontSize: "11px", color: "var(--up-text-dim)", textTransform: "uppercase", letterSpacing: "0.1em", display: "block", marginBottom: "5px" }}>Notas internas</label>
              <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} rows={4} placeholder="Anotações visíveis só para você..."
                style={{ width: "100%", background: "var(--up-card)", border: `1px solid var(--up-border)`, borderRadius: "6px", padding: "9px", fontSize: "12px", color: "#ccc", resize: "vertical", fontFamily: "inherit", lineHeight: 1.5, boxSizing: "border-box" }} />
