  ...
  return (
    <>
      ...
      <style>{`
        ...
        .kanban-col { min-width: 260px; max-width: 260px; display: flex; flex-direction: column; background: #0d0d0d; border: 1px solid ${BORDER}; border-radius: 10px; overflow: hidden; }
        .kanban-col-header { padding: 14px 16px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid ${BORDER}; }
        ...
        .kanban-card { background: #161616; border: 1px solid ${BORDER}; border-radius: 8px; padding: 12px; cursor: pointer; transition: border-color 0.15s; user-select: none; }
        ...
        .add-card-btn { width: 100%; background: transparent; border: none; color: #444; font-size: 12px; padding: 8px; cursor: pointer; display: flex; align-items: center; gap: 6px; justify-content: center; border-top: 1px solid ${BORDER}; transition: color 0.15s; }
        .add-card-btn:hover { color: #888; }
        .detail-panel { position: fixed; top: 0; right: 0; width: 380px; height: 100vh; background: #111; border-left: 1px solid ${BORDER}; z-index: 100; display: flex; flex-direction: column; }
      ...
      <div className="kanban-wrap">
        <div style={{ marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <h2 style={{ fontSize: "24px", fontWeight: "700", color: "#F0EDE8", margin: 0, letterSpacing: "-0.02em" }}>Kanban</h2>
          </div>
          <div style={{ fontSize: "12px", color: "#555" }}>{totalCards} cards no funil</div>
        </div>

        {loading ? (
          <div style={{ color: "#555", fontSize: "13px", padding: "40px" }}>Carregando...</div>
        ) : (
                ...
                return (
                  <div key={col.id} className="kanban-col">
                    <div className="kanban-col-header">
                      ...
                      <span style={{ fontSize: "11px", color: "#555", background: "#1a1a1a", padding: "2px 7px", borderRadius: "10px" }}>{cards.length}</span>
                    </div>
                          ...
                          {cards.map((card, index) => (
                            <Draggable key={card.id} draggableId={card.id} index={index}>
                              {(provided, snapshot) => (
                                ...
                                >
                                  <div style={{ fontWeight: "600", fontSize: "13px", color: "#F0EDE8", marginBottom: "6px", lineHeight: 1.3 }}>{card.nome}</div>
                                  {card.cidade && (
                                    <div style={{ display: "flex", alignItems: "center", gap: "4px", marginBottom: "4px" }}>
                                      <MapPin size={10} color="#555" />
                                      <span style={{ fontSize: "11px", color: "#666" }}>{card.cidade.split(",")[0]}</span>
                                    </div>
                                  )}
                                  {card.tipo && (
                    ...
                    {addingTo === col.id ? (
                      <div style={{ padding: "8px 10px", borderTop: `1px solid ${BORDER}` }}>
                        <input
                          ...
                          style={{ width: "100%", background: "#1a1a1a", border: `1px solid ${ACCENT}`, borderRadius: "6px", padding: "8px 10px", fontSize: "12px", color: "#F0EDE8", outline: "none", boxSizing: "border-box" }}
                        />
                        <div style={{ display: "flex", gap: "6px", marginTop: "6px" }}>
                          ...
                          <button onClick={() => setAddingTo(null)} style={{ background: "transparent", border: `1px solid ${BORDER}`, borderRadius: "5px", padding: "6px 8px", cursor: "pointer", color: "#555" }}><X size={12} /></button>
                        </div>
                      </div>
                    ) : (
      ...
      {selected && (
        <div className="detail-panel">
          <div style={{ padding: "20px 24px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <p style={{ fontSize: "10px", color: "#555", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.1em" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
            {[
              ...
              <div key={label} style={{ marginBottom: "16px" }}>
                <p style={{ fontSize: "10px", color: "#555", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</p>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <div style={{ marginTop: "8px" }}>
              <p style={{ fontSize: "10px", color: "#555", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Notas</p>
              <textarea
                value={editNotas}
                onChange={(e) => setEditNotas(e.target.value)}
                placeholder="Adicione notas sobre esse lead..."
                rows={5}
                style={{ width: "100%", background: "#0d0d0d", border: `1px solid ${BORDER}`, borderRadius: "8px", padding: "12px", fontSize: "13px", color: "#ccc", outline: "none", resize: "vertical", fontFamily: "inherit", lineHeight: 1.6, boxSizing: "border-box" }}
              />
              <button
            ...
            <div style={{ marginTop: "24px", paddingTop: "20px", borderTop: `1px solid ${BORDER}` }}>
              <p style={{ fontSize: "10px", color: "#555", margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Mover para coluna</p>
              <select
                ...
                style={{ width: "100%", background: "#1a1a1a", border: `1px solid ${BORDER}`, borderRadius: "6px", padding: "10px 12px", fontSize: "13px", color: "#ccc", outline: "none" }}
              >
