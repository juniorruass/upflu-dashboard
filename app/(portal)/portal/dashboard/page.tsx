...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  draft:    { label: "Rascunho", color: "var(--up-text-label)", bg: "rgba(119,112,104,0.1)", icon: Edit2 },
...
export default function PortalDashboard() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "var(--up-bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: "var(--up-text-label)", fontSize: "13px" }}>Carregando...</p>
    </div>
  );
  if (error || !data) return (
    <div style={{ minHeight: "100vh", background: "var(--up-bg)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "12px" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--up-bg)", fontFamily: "var(--font-outfit, sans-serif)" }}>

      {/* Header */}
      <div style={{ borderBottom: `1px solid var(--up-border)`, padding: "0 40px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "60px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <span style={{ fontSize: "13px", fontWeight: "700", color: ACCENT, letterSpacing: "0.1em" }}>UPFLU</span>
            <span style={{ fontSize: "11px", color: "var(--up-text-label)" }}>/ Portal do Cliente</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
            <span style={{ fontSize: "13px", color: "var(--up-text-muted)" }}>{client.name}</span>
            <button onClick={logout} style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "11px", color: "var(--up-text-label)", background: "transparent", border: `1px solid var(--up-border)`, borderRadius: "6px", padding: "6px 10px", cursor: "pointer" }}>
              <LogOut size={12} /> Sair
            </button>
          </div>
        </div>
      ...
      <div style={{ padding: "40px", maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          ...
          <h1 style={{ fontSize: "26px", fontWeight: "700", color: "var(--up-text)", margin: "0 0 4px", letterSpacing: "-0.03em" }}>{client.name}</h1>
          <p style={{ fontSize: "13px", color: "var(--up-text-label)", margin: 0 }}>{client.segment} · desde {client.start_date ? fmtDate(client.start_date) : "—"}</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "14px", marginBottom: "32px" }}>
          {[
            ...
            <div key={s.label} style={{ background: BG, border: `1px solid var(--up-border)`, borderRadius: "10px", padding: "20px", position: "relative", overflow: "hidden" }}>
              ...
              <p style={{ fontSize: "10px", fontWeight: "500", color: "var(--up-text-label)", margin: "0 0 8px", letterSpacing: "0.14em", textTransform: "uppercase" }}>{s.label}</p>
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
            ...
            <h2 style={{ fontSize: "14px", fontWeight: "600", color: "var(--up-text)", margin: 0 }}>Propostas & Contratos</h2>
          </div>
          {proposals.length === 0 ? (
            <p style={{ fontSize: "13px", color: "var(--up-text-label)" }}>Nenhum documento ainda.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {proposals.map(p => {
                ...
                const Icon = cfg.icon;
                return (
                  <div key={p.id} style={{ background: BG, border: `1px solid var(--up-border)`, borderRadius: "10px", padding: "16px 20px", display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px", flexWrap: "wrap" }}>
                        <span style={{ fontSize: "13px", fontWeight: "600", color: "var(--up-text)" }}>{p.title}</span>
                        ...
                        <span style={{ fontSize: "10px", color: "var(--up-text-label)", background: "rgba(255,255,255,0.04)", border: `1px solid var(--up-border)`, padding: "2px 7px", borderRadius: "4px" }}>
                      ...
                      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: "12px", color: "var(--up-text-label)" }}>criado {fmtDate(p.created_at)}</span>
                      </div>
                    </div>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
            ...
            <h2 style={{ fontSize: "14px", fontWeight: "600", color: "var(--up-text)", margin: 0 }}>Histórico de Pagamentos</h2>
          </div>
          {payments.length === 0 ? (
            <p style={{ fontSize: "13px", color: "var(--up-text-label)" }}>Nenhum pagamento registrado.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {payments.map(p => {
                const st = paymentStatus(p);
                return (
                  <div key={p.id} style={{ background: BG, border: `1px solid var(--up-border)`, borderRadius: "10px", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
                    <div>
                      <p style={{ fontSize: "13px", fontWeight: "600", color: "var(--up-text)", margin: "0 0 3px" }}>{currency(p.amount)}</p>
                      <p style={{ fontSize: "12px", color: "var(--up-text-label)", margin: 0 }}>
